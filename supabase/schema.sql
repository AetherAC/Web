...
-- Stripe, PayPal, PayerURL 和 Alipay 都由 api/_lib/payments.mjs 驱动，而不是走可配置的 create_url
-- 路径，所以 public_config.driver 才是让它们能用的那一项。下面的 jsonb 拼接保留行里已有的键（尤其是
-- paypal.environment='sandbox' 和 alipay.product），重跑 schema 不会把它们打回默认。
