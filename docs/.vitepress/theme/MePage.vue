...
import SiteHeader from './SiteHeader.vue';import SiteFooter from './SiteFooter.vue';import {supabase,useAuth} from './auth';import {orderPath} from './routes'
...
<template><div class="fluent-page"><SiteHeader/><main class="account-main"><section class="account-head"><div class="account-avatar"><UserRound/></div><div><p>我的账户</p><h1>{{auth.user.value?.user_metadata?.display_name||auth.user.value?.user_metadata?.name||auth.user.value?.user_metadata?.username||auth.user.value?.email||'AetherAC 用户'}}</h1><span>{{auth.group.value}} · {{auth.user.value?.email_confirmed_at?'邮箱已验证':'邮箱未验证'}}</span></div><button @click="auth.signOut"><LogOut :size="17"/>退出</button></section><section class="account-metrics"><article><CreditCard/><span>累计消费</span><strong>{{money(total(),orders[0]?.currency||'USD')}}</strong></article><article><ReceiptText/><span>订单数量</span><strong>{{orders.length}}</strong></article><article><Package/><span>已完成购买</span><strong>{{orders.filter(x=>x.status==='paid').length}}</strong></article></section><LdcPanel/><section class="order-history"><header><div><p>ORDER HISTORY</p><h2>购买记录</h2></div><a href="/buy">购买 Artifact <ArrowUpRight :size="16"/></a></header><p v-if="message" class="order-message">{{message}}</p><div class="order-table"><div v-for="o in orders" :key="o.id" class="order-row"><a :href="orderPath(o.id)"><span><b>{{o.sku}}</b><small>{{new Date(o.created_at).toLocaleString('zh-CN')}}</small></span><span>{{o.provider}}</span><span>{{money(o.amount_minor,o.currency)}}</span><em :class="`order-${o.status}`">{{o.status}}</em><ArrowUpRight :size="16"/></a><button v-if="o.status==='pending'" class="cancel-order" :disabled="cancelling===o.id" @click="cancel(o)"><XCircle :size="15"/>{{cancelling===o.id?'取消中…':'取消订单'}}</button></div><div v-if="!orders.length" class="fluent-empty">还没有订单。</div></div><small v-if="orders.some(o=>o.status==='pending')" class="pending-note">每个账户同时只能有 1 笔待支付订单。要买别的构建，请先完成或取消上面那笔。</small></section></main><SiteFooter/></div></template>
