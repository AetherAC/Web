...
import AdminLdc from './AdminLdc.vue'
...
const auth = useAuth()
type Tab = 'overview'|'posts'|'progress_entries'|'orders'|'refunds'|'coupons'|'auto_replies'|'ldc'|'repositories'|'artifacts'|'payment_providers'|'user_profiles'|'site_settings'|'environment'
...
const tabs=computed(()=>[
  ...
  ...(auth.isAdmin.value?[
    {id:'ldc',label:'Linux.DO / LDC',icon:BadgePercent},
...
async function load(){
  ...
  if(tab.value==='orders'||tab.value==='refunds'||tab.value==='coupons'||tab.value==='auto_replies'||tab.value==='ldc'){ childKey.value++; return }
...
<template v-else-if="tab==='ldc'"><AdminLdc :key="childKey"/></template>
