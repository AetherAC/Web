...
import { Ban, BadgePercent, BarChart3, Boxes, CreditCard, FileText, Github, GitBranch, KeyRound, LayoutDashboard, Plus, ReceiptText, RefreshCw, Save, Settings, ShieldCheck, Trash2, UserX, Users } from 'lucide-vue-next'
...
const auth = useAuth()
type Tab = 'overview'|'posts'|'progress_entries'|'orders'|'coupons'|'repositories'|'artifacts'|'payment_providers'|'user_profiles'|'site_settings'|'environment'
...
const tabs=computed(()=>[
  ...
  ...(auth.isStaff.value?[{id:'coupons',label:'优惠券',icon:BadgePercent}]:[]),
  ...(auth.isAdmin.value?[
...
async function load(){
  ...
  if(tab.value==='orders'||tab.value==='coupons'){ childKey.value++; return }
