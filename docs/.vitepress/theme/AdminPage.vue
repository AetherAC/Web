...
import { Ban, BarChart3, Boxes, CreditCard, FileText, GitBranch, KeyRound, LayoutDashboard, Plus, RefreshCw, Save, Settings, ShieldCheck, Trash2, UserX, Users } from 'lucide-vue-next'
...
<template v-else-if="tab==='user_profiles'"><div class="fluent-notice compact"><b>四个用户组分别能做什么</b><p><code>default</code> 只能看公开内容和自己的订单，进不了这个后台。<code>read</code> 能额外看到草稿文章和已停用的仓库，但不能改任何东西。<code>coworker</code> 可以增删改 Blog / News 和开发进度。<code>admin</code> 拥有全部权限，包括 GitHub 仓库、Artifact 与价格、支付平台、网站设置、环境变量和这个用户管理页。权限由 Supabase RLS 在数据库层强制，不是靠前端藏按钮。</p></div><div class="admin-grid"><section class="records"><div class="section-toolbar"><span>{{rows.length}} 个账号 · {{userStats.admins}} 名管理员</span><button @click="loadUsers"><RefreshCw :size="17"/>刷新</button></div><div v-if="loading" class="fluent-empty">加载中…</div><article v-for="row in rows" :key="row.user_id" :class="{selected:selected&&selected.user_id===row.user_id}" @click="choose(row)"><div><b>{{row.email||row.display_name||row.user_id}}</b><small>{{row.group_name}}<template v-if="row.github_login"> · @{{row.github_login}}</template><template v-if="row.orders"> · {{row.orders}} 单</template><template v-if="row.banned"> · 已封禁</template><template v-if="row.profile_missing"> · 无 profile 行</template><template v-if="row.self"> · 当前登录</template></small></div><select :value="row.group_name" @click.stop @change="setGroup(row,$event)"><option v-for="g in ['default','read','coworker','admin']" :key="g">{{g}}</option></select><button :title="row.banned?'解除封禁':'封禁账号'" @click.stop="toggleBan(row)"><component :is="row.banned?ShieldCheck:Ban" :size="16"/></button><button title="删除账号" @click.stop="deleteUser(row)"><UserX :size="16"/></button></article></section>
<section v-if="selected" class="record-editor"><div class="editor-title"><div><p>USER / {{selected.user_id}}</p><h2>{{selected.email||selected.display_name||'未命名账号'}}</h2></div></div><div class="metric-grid"><article><span>用户组</span><strong>{{selected.group_name}}</strong><small>{{selected.profile_missing?'profile 行缺失，改组时自动补建':'由 Supabase RLS 强制执行'}}</small></article><article><span>订单 / 退款</span><strong>{{selected.orders}} / {{selected.refunds}}</strong><small>两者均为 on delete restrict</small></article><article><span>账号状态</span><strong>{{selected.banned?'已封禁':'正常'}}</strong><small>{{selected.email_confirmed?'邮箱已验证':'邮箱未验证'}}</small></article></div><div class="admin-callout compact"><b>登录信息</b><p>登录方式：{{selected.providers?.length?selected.providers.join('、'):'—'}}<br>注册时间：{{selected.created_at}}<br>最后登录：{{selected.last_sign_in_at||'从未登录'}}</p></div><div class="editor-row"><button class="fluent-secondary" @click="toggleBan(selected)"><component :is="selected.banned?ShieldCheck:Ban" :size="17"/>{{selected.banned?'解除封禁':'封禁账号'}}</button></div><label class="check-label"><input v-model="cascade" type="checkbox">连带删除该账号的订单与退款记录（不可恢复）</label><button class="fluent-primary" @click="deleteUser(selected)"><UserX :size="17"/>永久删除此账号</button><div class="admin-callout compact"><b>删不掉的时候</b><p>唯一的管理员账号、以及当前登录的账号会被服务端拒绝，避免把自己锁在后台之外。存在交易记录的账号需要先勾选连带删除。</p></div></section><div v-else class="fluent-empty"><Users :size="28"/><h2>选择一个账号查看详情</h2></div></div></template>
