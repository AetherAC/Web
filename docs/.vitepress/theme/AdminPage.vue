...
import { Ban, BarChart3, Boxes, CreditCard, FileText, Github, GitBranch, KeyRound, LayoutDashboard, Plus, ReceiptText, RefreshCw, Save, Settings, ShieldCheck, Trash2, UserX, Users } from 'lucide-vue-next'
...
const auth = useAuth()
type Tab = 'overview'|'posts'|'progress_entries'|'orders'|'repositories'|'artifacts'|'payment_providers'|'user_profiles'|'site_settings'|'environment'
...
// 订单那一页自己管数据。这个 key 只在点右上角刷新时 +1，用来把组件整个重挂——它内部有筛选条件和
// 打开的详情，与其从外面伸手去调它的 load()，不如让 Vue 重建一次。
...
async function load(){
  ...
  if(tab.value==='orders'){ ordersKey.value++; return }
...
<template v-else-if="tab==='orders'"><AdminOrders :key="ordersKey"/></template>
